...
from mnoptical.node import Transceiver, Roadm, LineTerminal, SignalTracing, Splitter, Roadm
...
noiseStart = 21 # which channel the 10 noise channels start on
...
osnr_power_at_nodes = 0 #Toggles intermediate checking graphs on and off (NOTE DO NOT SET BOTH THIS AND PLOT RESULTS TO 1)
...
#channel map - elements in map are boosted through the roadms , else signals are just through (NOTE to Ashley check your understanding of this)
channel_map_this = set()
for i in range(TXCOUNT):
    if (noiseStart + i) != CH5ALICE:
        channel_map_this.add(noiseStart + i)
...
testfile = open('OSNR_test.txt', 'w')
optical_signal_list = open('Opsig-test.txt','w')
...
def createnetwork(power_a, length_bga, num_ab_spans, length_ab_spans, 
    ...
    r0 = net.add_roadm('r0', monitor_mode='out', channel_map=channel_map_this)
    ...
    r1 = net.add_roadm('r1', monitor_mode='out', channel_map=channel_map_this)
    r2 = net.add_roadm('r2', monitor_mode='in', channel_map=channel_map_this)
    ...
    for r in range(1, num_ab_roadms + 1):
        r_ab.append(net.add_roadm(f'r_ab_{r}', monitor_mode='in', channel_map=channel_map_this))
...
def run(update_net_plot, plot_willie_signals, plot_r2_signals, plot_t2_signals, 
    ...
    for n in modes_span:
        ...
        while ((RE_budget_ratio < (1 - RE_margin)) or (power_fidelity > 0)) and (twin_test_counter < max_twin_tests):
            ...
            print(f'\n --------  ITERATION {twin_test_counter + 1:.0f} for {n:.0f} modes for taploc{tap_loc}  --------')
        ...
        if( n == 100000):    
            testfile.write(str(dict({'power': power_a_test})))
            testfile.write('\n')
            testfile.write(str(dict({'tap location': tap_loc})))
            testfile.write('\n')
            testfile.write(str(dict({'Willie OSNR': willie_input_osnr_alice_db})))
            testfile.write('\n\n')
...
# ___________________ v--  beginning of "real" network  --v ______________________
    ...
    for i, power_a in enumerate(power_a_test_list_db): # need to loop through both power and corresponing n <<<<<<<<<<
        ...
        if(CH5ALICE <= noiseStart):
            ...
            willie_input_osnr_alice_db = willie_input_osnr_dB_list[10][1]
        # Debugging: check OSNRs at an amp_ab amp. Tap replaces one, so key error if you try to check that one.
        if((tap_loc == 2)and (n==100000)and (osnr_power_at_nodes==1)):
            flag_ab = 1
            for r in range(1, num_ab_roadms+1):
                amp_ab = net.name_to_node[f'r_ab_{r}']
                amp_ab_list = amp_ab.monitor.get_list_osnr()
                amp_ab_pows = getsignalwatts(amp_ab)
                if(CH5ALICE <= noiseStart):
                    amp_db = amp_ab_list[0][1]
                    amp_db_noise = amp_ab_list[1][1]
                    amp_pow = wtodbm(amp_ab_pows[CH5ALICE]['aseW'])
                elif(CH5ALICE == noiseStart+4):
                    amp_db = amp_ab_list[4][1]
                    amp_db_noise = amp_ab_list[1][1]
                    amp_pow = wtodbm(amp_ab_pows[CH5ALICE]['aseW'])
                else:
                    amp_db = amp_ab_list[10][1]
                    amp_db_noise = amp_ab_list[1][1]
                    amp_pow = wtodbm(amp_ab_pows[CH5ALICE]['aseW'])
                osnr_list.append(amp_db)
                osnr_list_noise.append(amp_db_noise)
                power_list.append(amp_pow)
                    #print(*onsr_list)

        if(flag_ab==1):
            print("To be plotted, OSNR List: \n", osnr_list)
            plt.plot(osnr_list,linestyle='dashed', marker='s')
            plt.xlabel("roadm number")
            plt.ylabel("onsr")
            plt.title("ONSR on alice")
            plt.show()

            plt.plot(osnr_list_noise,linestyle='dashed', marker='s')
            plt.xlabel("roadm number")
            plt.ylabel("onsr")
            plt.title("ONSR on noise")
            plt.show()

            plt.plot(power_list,linestyle='dashed', marker='s')
            plt.xlabel("roadm number")
            plt.ylabel("pwr")
            plt.title("power on noise alice")
            plt.show()
        ...
        r0 = net.name_to_node['r0']

        #optical_signal_list.write(str(dict({'r0_list' : r0.get_optical_signals(self)})))
        #optical_signal_list.write('\n\n')
...
def plotrunresults(modes_span, power_a_test_list_db, nRE_budget, RE_margin, 
    ...
    #plt.ylim(.8*nRE_budget, 1.1*max(nRE_list))
    plt.ylim(0, 0.0050)
